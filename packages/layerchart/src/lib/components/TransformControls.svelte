...
>
  ...
  {#if show.includes('zoomOut')}
    <Tooltip title="Zoom out">
      <Button
        icon={LucideZoomOut}
        on:click={() => ctx.transform.zoomOut()}
        {size}
        class="text-surface-content p-2"
      />
    </Tooltip>
  ...
  {#if show.includes('center')}
    <Tooltip title="Center">
      <Button
        icon={LucideFocus}
        on:click={() => ctx.transform.translateCenter()}
        {size}
        class="text-surface-content p-2"
      />
    </Tooltip>
  ...
  {#if show.includes('scrollMode')}
    <Tooltip title="Scroll mode">
      <MenuButton
        ...
        menuIcon={null}
        {size}
        value={ctx.transform.scrollMode}
        on:change={(e) => ctx.transform.setScrollMode(e.detail.value)}
