...
export class HighlightState {
  ...
  points = $derived.by<HighlightPoint[]>(() => {
    ...
    if (
      ...
      const spans = rows.some((row: any) => Array.isArray(value(row)));
