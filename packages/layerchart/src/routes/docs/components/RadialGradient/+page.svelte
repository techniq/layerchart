...
<Preview>
  <div class="h-[334px] p-4 border rounded">
    <Chart>
      <Svg>
        <RadialGradient stops={['hsl(60 100% 50%)', 'hsl(30 100% 40%)']} let:url>
          <circle cx={radius + 0 * 120} cy={radius} r={radius} fill={url} />
        </RadialGradient>

        <RadialGradient stops={['hsl(60 100% 50%)', 'hsl(140 100% 40%)']} fx="20%" fy="20%" let:url>
          <circle cx={radius + 1 * 120} cy={radius} r={radius} fill={url} />
        </RadialGradient>

        <RadialGradient stops={['hsl(195 100% 50%)', 'hsl(270 100% 30%)']} r="30%" let:url>
          <circle cx={radius + 2 * 120} cy={radius} r={radius} fill={url} />
        </RadialGradient>
      </Svg>
    </Chart>
  </div>
...
<Preview>
  <div class="h-[252px] p-4 border rounded">
    <Chart>
      <Svg>
        <RadialGradient class="from-green-500 to-blue-500" r="30%" spreadMethod="pad" let:url>
          <circle cx={radius + 0 * 120} cy={radius} r={radius} fill={url} />
        </RadialGradient>

        <RadialGradient class="from-green-500 to-blue-500" r="30%" spreadMethod="reflect" let:url>
          <circle cx={radius + 1 * 120} cy={radius} r={radius} fill={url} />
        </RadialGradient>

        <RadialGradient class="from-green-500 to-blue-500" r="30%" spreadMethod="repeat" let:url>
          <circle cx={radius + 2 * 120} cy={radius} r={radius} fill={url} />
        </RadialGradient>
      </Svg>
    </Chart>
      ...
      <Svg>
        <RadialGradient class="from-green-500 to-blue-500" units="objectBoundingBox" let:url>
          {#each { length: 6 } as _, i}
            <circle cx={radius + i * 120} cy={radius} r={radius} fill={url} />
          {/each}
        </RadialGradient>

        <RadialGradient class="from-green-500 to-blue-500" units="userSpaceOnUse" let:url>
          {#each { length: 6 } as _, i}
            <circle cx={radius + i * 120} cy={120 + radius} r={radius} fill={url} />
          {/each}
        </RadialGradient>
      </Svg>
    </Chart>
