<script lang="ts">
	...
	import type { ComponentAPI, PropertyInfo } from '@layerstack/docs/api';
	...
	const apis = $derived((metadata.apis as ApiEntry[] | undefined) ?? []);
...
{#if apis.length}
	...
	{#each apis as entry (entry.component)}
		{#if apis.length > 1}
			<H3 id={`api-${entry.component}`}>{entry.label}</H3>
		{/if}
		...
		{#if entry.extends?.length}
			<div class="mt-4">
				...
				<div class="inline-flex gap-2">
					{#each entry.extends as extended}
