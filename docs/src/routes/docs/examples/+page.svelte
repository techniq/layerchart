			...
			<div class="grid grid-cols-2 sm:grid-cols-3 gap-4">
				{#each examples as example}
					<a
						...
						class="group block border border-surface-content/10 bg-surface-100 rounded-xl overflow-hidden hover:border-primary transition-colors"
					>
						{#if example.hasLightScreenshot || example.hasDarkScreenshot}
							<div class="aspect-3/2 p-0.5 overflow-hidden">
								<img
									src="/screenshots/{component}/{example.name}-light.png"
									alt="{component} - {example.name}"
									class="w-full h-full object-cover object-top dark:hidden border rounded-lg"
									loading="lazy"
								/>
								<img
									src="/screenshots/{component}/{example.name}-dark.png"
									alt="{component} - {example.name}"
									class="w-full h-full object-cover object-top hidden border rounded-lg dark:block"
									loading="lazy"
								/>
							</div>
						{:else}
