<script lang="ts">
  ...
  import { getWorldGeojson, getUsStatesTopology, getTimezones } from '$lib/data.remote.js';

  const worldTopojson = await getWorldGeojson();
  ...
  const countriesGeojson = $derived(
    feature(worldTopojson, worldTopojson.objects.countries)
  );
  const statesGeojson = $derived(
    feature(statesTopojson, statesTopojson.objects.states)
  );
  const timezoneGeojson = $derived(
    feature(timezonesTopojson, timezonesTopojson.objects.timezones)
  );
  ...
  const data = { worldTopojson, statesTopojson, timezonesTopojson, countriesGeojson, statesGeojson, timezoneGeojson };
